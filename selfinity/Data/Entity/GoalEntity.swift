...
struct GoalsEntity: Mappable {
    var items: [GoalEntity] = []
    
    init?(map: Map) { }
    
    init() { }
    
    init(items: [GoalEntity]) {
        self.items = items
    }
...
struct GoalEntity: Mappable {
    ...
    init(snapshot:DataSnapshot) {
        ...
        //self.steps                   = snapshot.childSnapshot(forPath: "steps/\(self.uid)").value as! String
        ...
        for child in snapshot.childSnapshot(forPath: "steps").children {
            guard let child = child as? DataSnapshot else {
                return
            }
            steps.append(StepEntity(snapshot: child))
        }
        self.steps = steps
    }
}
